...
JAR_DIR=$PWD/../../jenkins-dependencies/lib

echo "Logging into IBM Bluemix Container Cloud Service.."
##PARSE_JAR=$PLUGIN_HOME/lib/json.jar
export CLASSPATH="$JAR_DIR/mfpfParser.jar:$JAR_DIR/json.jar:$JAR_DIR/httpmime-4.5.jar:$JAR_DIR/commons-codec-1.9.jar:$JAR_DIR/commons-logging-1.2.jar:$JAR_DIR/httpclient-4.5.jar:$JAR_DIR/httpcore-4.4.1.jar"

auth_info_url="$BLUEMIX_API_URL/v2/info"
auth_endpoint=$(java com.ibm.mfpf.container.ManageMFPServerContainer 'retrieveAuthEndpointURL' "$BLUEMIX_API_URL/v2/info")

uaa_endpoint="$auth_endpoint/oauth/token"
echo "uaa_endpoint is $uaa_endpoint"
access_token=$(java com.ibm.mfpf.container.ManageMFPServerContainer 'login' "$auth_endpoint/oauth/token" 'password' $BLUEMIX_USER $BLUEMIX_PASSWORD)
echo "access_token in script is $access_token"

spaceGUID=$(java com.ibm.mfpf.container.ManageMFPServerContainer 'getSpaceGUID' "$BLUEMIX_API_URL/v2/spaces" $access_token $BLUEMIX_SPACE)
